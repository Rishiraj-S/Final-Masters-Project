...
def _compute_player_stats(events: pd.DataFrame, player_name: str) -> dict:
    ...
    shots      = exclude_own_goals(pe[pe['event_type'].isin(_SHOT_TYPES)].copy())
...
def build_player_analysis_tab(events: pd.DataFrame, **_) -> html.Div:
    """Build the Player Analysis tab: side-by-side all players + full stats table."""
    if events.empty:
...
def register_player_analysis_callbacks(app) -> None:

"""
Opposition Analysis — Chance Creation & Finishing
...
def _add_sp_flag(shots: pd.DataFrame, opp_ev: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    A shot is flagged True when EITHER:
      ...
      (b) Indirect: the immediately preceding event (same match) is a
    ...
    present_sp_pass = [c for c in _SP_PASS_COLS if c in opp_ev.columns]
...
def _get_opp_shots(opp_ev: pd.DataFrame) -> pd.DataFrame:
    """Return opposition shot rows with xG and _is_sp_shot columns."""
    shots = opp_ev[opp_ev['event_type'].isin(_SHOT_TYPES)].copy()
    shots = shots.dropna(subset=['x', 'y'])
    if shots.empty:
        return shots
    shots = add_xg_column(shots)
    shots = _add_sp_flag(shots, opp_ev)
    return shots


def _compute_key_pass_stats(opp_ev: pd.DataFrame) -> pd.DataFrame:
    """
    Vectorised: for each pass immediately followed by a shot (same match),
    ...
    sort_cols = [c for c in ['match_id', 'period_id', 'time_min', 'time_sec']
                 if c in opp_ev.columns]
    te = opp_ev.sort_values(sort_cols).reset_index(drop=True)
...
def _get_key_passes_for_map(opp_ev: pd.DataFrame, map_shots: pd.DataFrame) -> pd.DataFrame:
    """
    Return opposition passes that are key passes for shots in map_shots.
    ...
    """
    if opp_ev.empty or map_shots.empty:
        return pd.DataFrame()
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                html.Div("Top Assisters",
                         style={**_SECTION_TITLE, 'fontSize': '0.75rem'}),
                html.Div(style={'marginBottom': '6px'}),
                html.Div(id='occ-assisters-table', children=[]),
            ], md=4, style={
...
def register_chance_creation_callbacks(app) -> None:
    """Wire occ- filter controls to the shot map, KPI bar, and stat tables."""

    @app.callback(
        Output('occ-kpi-bar',           'children'),
        Output('occ-shot-map',          'figure'),
        Output('occ-zone-map',          'src'),
        ...
        Input('occ-show-kp',         'value'),
        Input('occ-zone-options',    'value'),
    ...
    def _update(players, outcomes, bands, h1_range, h2_range, show_kp,
        ...
        all_shots = _get_opp_shots(opp_ev)
        if all_shots.empty:
            return _empty()

        # KPI shots: outcome/method filters excluded so stats reflect full picture
        ...
        # Key pass stats for KPIs and assister table
        kp_stats = _compute_key_pass_stats(opp_ev)
        ...
        kp_for_map = (
            _get_key_passes_for_map(opp_ev, map_shots)
            if show_kp else None
        )
        carry_lines_data = (
            _get_carry_lines(opp_ev, map_shots)
            if show_kp else []
        )
        ...
        if not map_shots.empty:
            zone_src = _threat_zone_image(map_shots, opp_ev, show_shot_zones)

"""
Transform match event data from partidos JSON.
Output: data/result/{league}/{season}/match_event/{date}_{home_code}_vs_{away_code}_{match_id}.parquet
"""
import pandas as pd
...
class MatchEventTransformer(BaseTransformer):
    ...
    def __init__(self, config: dict, logger):
        ...
        if not Path(mappings_dir).is_absolute():
            mappings_dir = str(script_dir / mappings_dir)
    ...
    def process_match_json(self, json_data: dict) -> pd.DataFrame:
        """Transform JSON dict → comprehensive event DataFrame."""
        ...
        if "Team Formation" in df.columns:
            df["formation"] = (
                df["Team Formation"].astype(str).map(self.formation_mapping).fillna("")
            )
    ...
    def _extract_match_info(self, json_data: dict) -> dict:
        ...
        for c in contestants:
            if c.get("position") == "home":
                home_team = c.get("officialName", "unknown")
                home_code = c.get("code", "UNK")
            elif c.get("position") == "away":
                away_team = c.get("officialName", "unknown")
        ...
        return {
            ...
            "date": date, "week": match_info_obj.get("week", "unknown"),
            "score": "", "venue": venue_obj.get("shortName", venue_obj.get("longName", "unknown")),
        }

    def _build_filename(self, match_id: str, match_info: dict, naming_config: dict) -> str:
        date_clean = self._clean_filename(match_info.get("date", "unknown"), naming_config)
        ...
        return f"{date_clean}_{home_code}_vs_{away_code}_{match_id}"

    def transform_event(self, match_id: str, data: dict) -> Optional[str]:
        """Transform one match JSON dict → match_event Parquet."""
        if self._output_exists(match_id, "match_event"):
            self.logger.debug(f"   ⏭️  match_event parquet exists: {match_id}")
            return "skipped"

        try:
            events_df = self.process_match_json(data)
            if events_df.empty:
                self.logger.warning(f"   ⚠️  No events: {match_id}")
                return None

            naming_config = self.config.get("output", {}).get("naming", {})
            match_info = self._extract_match_info(data)
            filename = f"{self._build_filename(match_id, match_info, naming_config)}.{self.output_format}"

            output_path = get_organized_path_reversed(
                self.base_result_dir, self.league_name, self.season,
                filename, subdirectory="match_event",
            )
    ...
    def transform_all(self) -> int:
        """Transform all partidos JSONs → match_event Parquets."""
        ...
        for match_id, data, _ in self._load_all_partidos():
            result = self.transform_event(match_id, data)
            if result:
                successful += 1

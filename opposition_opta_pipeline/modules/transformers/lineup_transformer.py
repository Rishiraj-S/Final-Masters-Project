"""
Extract lineup/formation data from partidos JSON.
Source:  opposition_data/{league}/{season}/partidos/{match_file}.json
Output:  data/result/{league}/{season}/lineup/{date}_{home_code}_vs_{away_code}_{match_id}.parquet

Reads typeId=34 (Team setp up) events — identical logic to opta_pipeline,
only the file-loading entry point changes.
"""
import pandas as pd
...
_Q_PLAYER_IDS      = 30
_Q_FORMATION_SLOTS = 131
_Q_JERSEY_NUMBERS  = 59
_Q_POSITION_CODES  = 44
_Q_CAPTAIN         = 194
_Q_FORMATION_CODE  = 130
...
_POSITION_BROAD: Dict[int, str] = {1: "GK", 2: "DEF", 3: "MID", 4: "FWD", 5: "Sub"}
...
class LineupTransformer(BaseTransformer):
    """Extracts per-player lineup rows from partidos JSON files."""
    ...
    def _extract_match_naming(self, match_info: dict) -> dict:
        ...
        return {"date": date, "home_team": home_team, "away_team": away_team,
                "home_code": home_code, "away_code": away_code}

    def transform_lineup(self, match_id: str, data: dict) -> Optional[str]:
        """Transform one match JSON dict → lineup Parquet."""
        if self._output_exists(match_id, "lineup"):
            self.logger.debug(f"   ⏭️  lineup parquet exists: {match_id}")
            return "skipped"
        ...
        try:
            ...
            if not events:
                self.logger.warning(f"   ⚠️  [lineup] No events: {match_id}")
                return None
            ...
            naming = self._extract_match_naming(match_info)

            setup_events = [ev for ev in events if ev.get("typeId") == 34]
            if not setup_events:
                self.logger.warning(f"   ⚠️  [lineup] No typeId=34 events: {match_id}")
                return None
            ...
            for ev in setup_events:
                ...
                raw_pos     = _get_qual(ev, _Q_POSITION_CODES)
                captain_id  = _get_qual(ev, _Q_CAPTAIN)
                form_code   = _get_qual(ev, _Q_FORMATION_CODE)

                if not raw_pids:
                    self.logger.warning(f"   ⚠️  [lineup] Missing player IDs ({team_name})")
                    continue
                ...
                position_codes   = _parse_int_list(raw_pos)    if raw_pos    else [5] * len(player_ids)
                formation_str    = _FORMATION_MAP.get(form_code, form_code)
            ...
            if not rows:
                self.logger.warning(f"   ⚠️  [lineup] No rows extracted: {match_id}")
                return None
            ...
            self.logger.error(f"   ❌ Lineup transform failed for {match_id}: {e}")
            import traceback
            self.logger.error(traceback.format_exc())
            return None

    def transform_all(self) -> int:
        """Transform all partidos JSONs → lineup Parquets."""
        partidos_dir = self._get_partidos_dir()
        if not partidos_dir.exists():
            self.logger.info(f"   [lineup] No partidos directory: {partidos_dir}")
            return 0

        json_files = sorted(partidos_dir.glob("*.json"))
        if not json_files:
            self.logger.info("   [lineup] No partidos JSON files found")
            return 0

        self.logger.info(f"🔄 [lineup] Found {len(json_files)} partidos file(s)")
        print(f"   👥 Lineups: processing {len(json_files)} file(s)...")
        successful = failed = 0

        for match_id, data, _ in self._load_all_partidos():
            result = self.transform_lineup(match_id, data)
            if result:
                successful += 1
            else:
                failed += 1
